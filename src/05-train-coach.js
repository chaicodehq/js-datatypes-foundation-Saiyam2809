...
export function findPassenger(passengers, name) {
  if (!Array.isArray(passengers) || typeof name !== "string") {
   ...
   return passengers.find(p => typeof p.name === "string" && p.name.toLowerCase() === searchName)}
}
...
export function getPassengerIndex(passengers, name) {
  if (!Array.isArray(passengers) || typeof name !== "string") {
    ...
    return passengers.findIndex(p => typeof p.name === "string" && p.name.toLowerCase() === nameLower)
  }
}
...
export function isAnyWaitlisted(passengers) {
  if (!Array.isArray(passengers) || passengers.length <= 0) {
   ...
   return passengers.some(p => typeof p.status === "string" && p.status.toLowerCase() === "waitlisted")
  }
}
...
export function areAllConfirmed(passengers) {
  if (!Array.isArray(passengers) || passengers.length <= 0) {
    ...
    return passengers.every(p => typeof p.status === "string" && p.status.toLowerCase() === "confirmed")
  }
}
...
export function getWaitlistedPassengers(passengers) {
  if (!Array.isArray(passengers)) {
    ...
    return passengers.filter(p => typeof p.status === "string" && p.status === "waitlisted")
  }
}
